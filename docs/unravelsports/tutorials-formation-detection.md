...
For the mathematical formulation and validation, see: [Bekkers (2025): EFPI: Elastic Formation and Position Identification in Football (Soccer)](https://arxiv.org/pdf/2506.23843)
...
The `` `every` `` parameter uses Polars’ `` `group_by_dynamic` `` syntax. See [Polars Documentation](https://docs.pola.rs/api/python/stable/reference/dataframe/api/polars.DataFrame.group_by_dynamic.html) for more options.
