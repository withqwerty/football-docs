...
{% hint style="info" %}
This endpoint returns fixtures for a **single date only**. If you need a date range, use [GET Fixtures by Date Range](https://docs.sportmonks.com/v3/endpoints-and-entities/endpoints/fixtures/get-fixtures-by-date-range) instead (maximum range: 100 days).

**Timezone note:** passing a `timezone` parameter on this endpoint doesn't just reformat the display, it actually shifts which date bucket a fixture falls into, since `{date}` is interpreted in that timezone rather than UTC. See [Timezone parameters on different endpoints](https://docs.sportmonks.com/v3/tutorials-and-guides/tutorials/timezone-parameters-on-different-endpoints) for a worked example of this behavior.
{% endhint %}
